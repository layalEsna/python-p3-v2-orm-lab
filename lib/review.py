...
#     #     new_reviwe = cls(year, summary, employee_id)
#     #     new_reviwe.save()
#     #     return new_reviwe
...
class Review:
    ...
    def year(self, year):
        if isinstance(year, int) and year >= 2000:
           self._year = year
            ...
            raise ValueError('Year must be an integer and year must be 2000 or greater')
    ...
    def save(self):
        """Insert a new row or update an existing row for the Review instance."""
        if self.id is None:  # New review
            ...
            CONN.commit()
            self.id = CURSOR.lastrowid
            type(self).all[self.id] = self  # Save to dictionary
